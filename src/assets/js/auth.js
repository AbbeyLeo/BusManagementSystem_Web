var Auth = {
	vars: {
		...
		limit_imp:document.getElementsByClassName('limit-imp'),
		inputt: [],
		option: {}
	},
	register(e) {
	...
	unShow(){
//		Auth.vars.limit_imp.classList += ' lowin-animated';
		...
		Auth.vars.login_btn.innerText = '获取验证码';

		Auth.setHeight(Auth.vars.lowin_wrapper_height - Auth.vars.password_group_height);
		for(var i = 0; i <Auth.vars.inputt.length; i++)
		{
			Auth.vars.inputt[i].readOnly="";
			Auth.vars.inputt[i].disabled="";
		}
//		Auth.vars.lowin_login.querySelector('form').setAttribute('action', Auth.vars.option.forgot_url);
	},
	show(){
//		Auth.vars.limit_imp.classList.remove('lowin-animated');
...
//		Auth.vars.limit_imp.classList += ' lowin-animated';
		...
		Auth.setHeight(Auth.vars.lowin_wrapper_height - Auth.vars.password_group_height);
		Auth.vars.lowin_login.querySelector('form').setAttribute('action', Auth.vars.option.forgot_url);
			console.log(Auth.vars.inputt.length);
		for(var i = 0; i <Auth.vars.inputt.length; i++)
		{
			console.log(Auth.vars.inputt[i].readOnly);
			console.log(Auth.vars.inputt[i].disabled);
		}
